...
import { ArrowLeftIcon, ChevronRight, Clock, Edit2, HeartHandshakeIcon, HelpCircle, LocateFixedIcon, Lock, MapPin, Trash2, UserIcon } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const menuItems = [
    { id: 'profile-details', label: 'Profile Details', icon: UserIcon },
    { id: 'address', label: 'Address', icon: MapPin },
    { id: 'favourite', label: 'Favourite', icon: HeartHandshakeIcon },
    { id: 'help-centre', label: 'Help Centre', icon: HelpCircle },
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
          ...
          <div className="max-w-9xl mx-auto px-6 py-8">
            ...
            {/* Left Sidebar */}
            <div className="w-96 shrink-0 lg:w-96 lg:shrink-0">
                {/* Profile Card */}
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0  mb-2 lg:shrink flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap lg:whitespace-normal transition-colors min-w-max lg:min-w-0 lg:w-full ${
                        isActive
                          ? 'bg-gray-50 border-b-2 lg:border-b-0 lg:border-l-4 border-sky'
                          : 'hover:bg-gray-50'
                      }`}
                    >
                      <div className="flex items-center gap-2 sm:gap-3">
                        <Icon size={18} className={`sm:w-5 sm:h-5 ${isActive ? 'text-sky' : 'text-gray-600'}`} />
                        <span className={`text-sm sm:text-base font-medium ${isActive ? 'text-sky' : 'text-gray-900'}`}>
                          {item.label}
                        </span>
                      </div>
