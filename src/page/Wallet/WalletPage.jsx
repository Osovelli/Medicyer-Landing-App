...
import { CreditIcon, DebitIcon, ReloadIcon, TopUpIcon } from '@/components/custom/Icons';
...
const getTransactionIcon = (type) => {
  switch (type) {
    case 'credit':
      return { icon: <CreditIcon size={20} />, color: '#34A85333' };
    case 'debit':
      return { icon: <DebitIcon size={20} />, color: '#FF000033' };
    case 'conversion':
      return { icon: <ReloadIcon size={20} />, color: '#BD8CBF33' };
    default:
...
export default function WalletPage({ balance = 345000, points = 2300, transactions = mockTransactions }) {
  ...
  const columns = [
    { key: ['amount', 'transaction'], label: 'Amount', sortable: true, render: (value, row) => {
        const isCredit = row.type === 'credit';
        const isConversion = row.type === 'conversion';
        const { icon, color } = getTransactionIcon(row.type);
        console.log('Rendering transaction: ', {row, value}); // Log for debugging
        return (
          <div className="flex items-center space-x-3">
            <span className={`p-2 flex items-center justify-center rounded-full bg-[${color}] bg-opacity-30`}>
              {icon}
            </span>
            <span className={`font-semibold `}>
              {isCredit || isConversion ? '+' : '-'}
    ...
    { key: 'Description', label: 'Type', render: (_, row) => {
        const { icon, color } = getTransactionIcon(row.description);
        return (
          <div className="flex items-center space-x-3">
           {/*  <div className={`p-2 rounded-full bg-[${color}] bg-opacity-30`}>
              {icon}
            </div> */}          
            <div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="my-6 space-y-6 px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Main Balance Card */}
            <div className="bg-linear-to-r from-[#252B61] to-[#4C58C7] rounded-2xl p-6 text-white relative overflow-hidden">
              <div className="absolute inset-0 opacity-10">
                <img src='/main balance.png' alt='wallet icon' className='object-contain' />
              ...
              <div className="relative text-left space-y-10 z-10">
                ...
                <Button
                onClick={handleTopUp}
                className="mt-2 bg-transparent border border-indigo-200 text-white hover:bg-indigo-50/20"
                variant={'icon'}
                >
                  <TopUpIcon />
                  Top Up
            ...
            {/* Points Card */}
            <div className="bg-[#A3DAC2] rounded-2xl p-6 relative overflow-hidden">
