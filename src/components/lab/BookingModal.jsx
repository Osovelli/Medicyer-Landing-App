...
import { MapIcon, ShareIcon, Telegram, Whatsapp } from "../custom/Icons";
...
function LabInfoCard({ lab }) {
  return (
    <div className="flex dark:bg-yellow-900/30 items-start gap-3">
      {/* Jasiri badge */}
      <div className="w-14 h-14 rounded-xl bg-[#1C2A3D] flex items-center justify-center flex-shrink">
          <img src="/jaziri logo dark.png" alt="Lab Icon" className="rounded-xl" />
      </div>

      <div className="flex-1 min-w-0">
        <p className="text-[11px] text-slate-400 dark:text-slate-500">Laboratory</p>
        <p className="text-sm font-bold text-sky dark:text-slate-100 truncate">
          {lab.name}
        </p>
      ...
      {/* Rating */}
      <Badge className="bg-[#F0D869] hover:bg-[#F0D869]/40 text-white text-[10px] font-normal px-2 py-2 rounded-full shrink">
        <span>
          <Star fill="white" className="w-3 h-3" />
        </span> 
...
function BookingDetails({ price, dateLabel, address }) {
  return (
    <div className="space-y-3 mt-4">
      ...
      <div className="flex items-center gap-3">
        <CalendarRangeIcon className="w-4 h-4 text-slate-400 shrink-0" />
...
function StepSchedule({ test, selectedDay, selectedTime, onDayChange, onTimeChange, onBook }) {
  ...
  return (
    <div className="flex flex-col mx-auto max-w-xs sm:max-w-none">
      {/* Test summary card */}
      {test !== null && test !== undefined && <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-2 ">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            <span className="w-10 h-10 rounded-md bg-[#A3DAC2] dark:bg-green-900/40 flex items-center justify-center">
              <MapPin fill="#f1f1f1" stroke='#A3DAC2' className="w-6 h-6" />
            </span>
      ...
      <div className="mt-2 pt-4 border-t border-slate-100 dark:border-slate-700">
        <CustomButton
          ...
          className="w-full rounded-lg hover:bg-[#1a1f4a] dark:bg-blue-600 dark:hover:bg-blue-500 text-white text-sm font-bold disabled:opacity-40"
          size="lg"
        >
          Book Appointment
        </CustomButton>
...
function StepConfirm({ lab, test, dateLabel, onCancel, onProceed }) {
  return (
    <div className="flex flex-col items-center mx-auto max-w-xs sm:max-w-none">
      {/* Warning icon */}
      <div className="w-24 h-24 rounded-full bg-[#BD8CBF26] dark:bg-purple-900/30 flex items-center justify-center mb-4">
        <AlertTriangle className="w-full h-full" />
      </div>

      <h3 className="text-xl font-bold text-sky dark:text-slate-100 mb-5">
      ...
      <div className="flex gap-3 w-full mt-6">
        <CustomButton
          ...
          size="xl"
          onClick={onCancel}
          className="flex-1 rounded-lg font-normal border border-sky dark:border-slate-600 text-[#353849] dark:text-slate-300 text-sm hover:bg-slate-50 dark:hover:bg-slate-700"
        >
          No, Cancel
        </CustomButton>
        <CustomButton
          size="xl"
          onClick={onProceed}
          className="flex-2 rounded-lg hover:bg-[#1a1f4a] dark:bg-blue-600 dark:hover:bg-blue-500 text-white font-normal text-sm"
        >
          Yes, Proceed
        </CustomButton>
      </div>
...
function StepConfirmed({ lab, test, dateLabel, bookingId, onSeeBookings, onHome }) {
  const socials = [
    { id: "telegram", label: "Telegram", icon: Telegram, color: "text-[#229ED9]" },
    ...
    { id: "other", label: "Other", icon: ShareIcon, color: "text-slate-500" },
  ];
    ...
    <div className="flex flex-col items-center text-center">
      ...
      <div className="flex items-center justify-center gap-6 mt-6 mb-6">
        {socials.map(({ id, label, icon: Icon, color }) => (
          <button key={id} className="flex flex-col items-center gap-1.5 group">
            <div className="w-18 h-18 rounded-full bg-[#F6F7F7] dark:bg-slate-700 flex items-center justify-center group-hover:bg-slate-200 dark:group-hover:bg-slate-600 transition-colors">
      ...
      <div className="flex gap-3 w-full">
        <CustomButton
          ...
          className="rounded-lg px-10 border-2 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700"
        >
          See Bookings
        </CustomButton>
        <CustomButton
          onClick={onHome}
          size="xl"
          className="flex-1 rounded-lg bg-sky hover:bg-[#1a1f4a] dark:bg-blue-600 dark:hover:bg-blue-500 text-white font-bold text-sm"
        >
          Home
        </CustomButton>
      </div>
...
export function BookingModal({
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className={cn(
          "w-[calc(100%-2rem)] xs:max-w-[480px] ] overflow-y-auto overflow-x-scroll scrollbar-hide p-2",
